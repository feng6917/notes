...
##### 9.2摘要-签名-PKI-访问控制-DOS-欺骗技术

-  信息摘要
  > 信息摘要，就是一段数据的特征信息，当数据发生了改变，信息摘要也会发生改变，发送方会将数据和信息摘要一起传给接收方，接收方会根据接收到的数据重新生成一个信息摘要，若此摘要和接收到摘要相同，则说明数据正确。信息摘要是由哈希函数生成的。

  > 信息摘要的特点，不管数据多长，都会生成固定长度的信息摘要；任何不同的输入数据，都会产生不同的信息摘要；单向性，即只能由数据生成信息摘要，不能由信息摘要还原数据。

  > 信息摘要算法：MD5（产生128位输出）、SHA-1（安全散列算法，产生160位的输出，安全性更高）
  ...
  > 数字证书： 一个可信任的权威机构签署的信息集合。 公钥证书主要用于确保公钥及其与用户绑定关系的安全。这个公钥就是证书所表示的那个主体的合法的公钥。
  > 签证机构CA：负责签发证书，管理和撤销证书。CA在给用户签发证书时要加上自己的数字签名，以保证证书信息的真实性。任何机构可以用CA的公钥来验证该证书的合法性。

- 访问控制
  > 访问控制是主体依据某些控制策略或权限对客体本身或是其资源进行的不同授权访问。访问控制包括三个要素，即主体，客体和控制策略。
  > 访问控制包括认证，控制策略实现和审计。

  > 访问控制矩阵，是通过矩阵形式表示访问控制规则和授权用户权限的方法。主体为行，客体为列。
  > 访问控制表（ACL），是最流行，使用最多的，按列（客体）保存。
  > 能力表，按行（主体）保存。
  > 授权关系表，每一行（或者说元组）就是访问矩阵中的一个非空元素，是某一个主体对应某一个客体的访问权限信息。

- 信息安全的抗攻击技术
  > 为对抗攻击者的攻击，密钥生成考虑三个方面，增大密钥空间，选择强钥（复杂的），密钥的随机性（使用随机数）

  > 拒绝服务的攻击由许多种，内外部用户都可以发动攻击。长时间占用系统的内存，CPU处理时间使其他用户不能及时得到这些资源，外部可以通过占用网络连接使其他用户得不到网络服务。
  > 外部，消耗资源，破坏或更改配置信息，物理破坏或改变网络部件，利用服务程序中的处理错误使服务失效。
  > 分布式拒绝服务DDos，首先侵入并控制一些计算机，然后控制这些计算机同时向一个特定的目标发起拒绝服务攻击。克服了受网络资源的限制和隐蔽性两大缺点。
  ...
  > ARP 欺骗， 主机A就广播发送ARP请求分组，局域网内主机都会收到，但只有B收到解析后知道请求自己的MAC地址，A收到相应分组后，会建立一个B的IP地址和MAC地址映射。
  > A 是不管其有没有发送过广播分组的，而是只要收到了返回的分组信息，就会刷新IP地址和MAC地址的映射关系。
  > 防护措施，1. 固化arp表 2. 双向绑定 

  > DNS七篇首先是冒充域名服务器，然后把查询的IP地址设为攻击者的IP地址，用户上网只能看到攻击者的主页。
  > 被动监听检测，通过旁路监听的方式，在一定的时间间隔内，一个请求对应两个或两个以上结果不同的应答包，则怀疑受到了DNS欺骗攻击。
  > 虚假报文探测，主动发送探测包，如果向一个非DNS服务器发送请求包，正常不会收到任何应答，如果收到了，则说明受到了攻击。
  > 交叉检查查询，反向查询。

  > IP欺骗的原理和流程
  > 1. 首先使被冒充主机的网络暂时瘫痪。
  > 2. 连接到目标机的某个端口来猜测ISN基值和增长规律。
  > 3. 把源地址伪装成被冒充主机，发送带有SYN标志的数据段请求连接。
  > 4. 等待目标及发送SYN——ACK包给已经瘫痪的主机，
  > 5. 发送 带有预测的目标机的ISN+1
  > 6. 建立连接，发送请求

##### 9.3端口扫描-安全体系-网络安全技术和协议
- 端口扫描 
  > 尝试与目标主机的某些端口建立连接，如果有回复，则说明该端口开放，即 活动端口。
  - 扫描原理分类
  > 全TCP连接
  > 半打开式扫描（SYN扫描），发送SYN数据段，表示发送建立连接请求。报文 SYN＝１　ACK＝１，　则说明该端口是活动的。回应是RST，则表示该端口　未开放。 
  > FIN扫描。依靠发送FIN来判断目标计算机的指定端口是否是活动的。发送一个FIN=1的TCP报文到一个关闭的端口时，该报文会被丢掉，并返回一个RST报文。如果报文发送到一个活动端口，不会返回任何反应。
  > 第三方扫描，利用第三方主机来代替入侵者进行扫描。 

  > 强化TCP/IP堆栈以抵御拒绝服务攻击
  > 1. 同步包风暴，利用TCP协议缺陷发送大量伪造的TCP连接请求，使得被攻击者资源耗尽。
  > 2. ICMP 攻击，“Ping of Death” 利用操作系统规定的ICMP数据包的最大尺寸不超过64kb规定，使TCP/IP堆栈崩溃，主机司机。 
  > 3. SNMP 攻击，用于控制这些设备和产品，重定向通信流，改变通信数据包的优先级，完全接管你的网络。

  >　计算机系统安全保护能力的五个等级
  > 1. 用户自主保护级，具备自主安全保护的能力。
  > 2. 系统审计保护级，粒度更细的自主访问控制，在自主访问控制的基础上控制访问权限扩散。
  > 3. 安全标记保护级，安全策略模型，数据标记以及主体对客体强制访问控制的非形式化描述。
  > 4. 结构化保护级，将第三级系统中那个的自主和强制访问控制扩展到所有主体与客体。考虑隐蔽通道。
  > 5. 访问验证保护级，满足访问监控需求，根据用户指定方式或默认方式，组织非授权用户访问客体。

  > 防火墙是在内部网络和外部因特网之间增加的一道安全防护措施，分为网络级防火墙和应用级防火墙。
  > 网格级防火墙层次低，但是效率高
  > 应用级防火墙层次高，效率低。

  > 入侵检测系统IDS 防火墙之后的第二道屏障
  > 原理，监控当前系统，用户行为
  > 不同于防火墙，IDS入侵检测系统是一个监听设备，没有跨接在任何链路上，无需网络流量流经它便可以工作，IDS应该挂接在所有所关注流量都必须流经的链路上。
  > 1. 尽可能靠近攻击源 2. 尽可能靠近受保护资源

  > 入侵防御系统IPS
  > IPS是能够提前发现入侵行为，在其还没有进入安全网络之前就防御。在安全网络之前的链路上挂载入侵防御系统IPS，可以实时监测入侵行为，并直接进行阻断。
...
- 物理层主要使用物理手段，隔离，屏蔽物理设备等，其他层都靠协议来保证传输的安全  
  ![img](10.jpg)

- SSL 协议， 加强Web安全传输（HTTP/HTTPS）的协议
- SSH 协议，安全外壳协议，加强Telnet/FTP 安全的传输协议。
- SET 协议，安全电子交易协议主要用于B2C模式（电子商务）中保障支付信息的安全性。
- Kerberos协议， 是一种网络身份认证协议，基于信任第三方，它提供了开放型网络中进行身份认证的方法。  
- PGP 协议， 使用RSA公钥证书进行身份认证，使用IDEA（128位密钥）进行数据加密，使用MD5进行完整性验证。
